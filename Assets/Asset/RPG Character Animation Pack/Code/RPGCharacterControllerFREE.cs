...
public class RPGCharacterControllerFREE : MonoBehaviour
{
	...
	bool isFalling;
	bool startFall;
	float fallingVelocity = -0.2f; //원래 -1f
	...
	void Update( )
	{
		...
		if( useNavMesh )
		{
			...
			{
				Debug.Log( "tt	    : " + tt );
				Debug.Log( "pathEnd	: " + agent.pathEndPosition );
				Debug.Log( "dest	: " + agent.destination );
				Debug.Log( "next	: " + agent.nextPosition );
				Debug.Log( "trans	: " + agent.transform.position );
				Debug.Log( "-------------" );
			}//if(  >agent.destination.y - jumpAvail )
			{
		...
		//if( agent.isOnOffMeshLink && !check )
		//{
		...
		//	Debug.Log( currPos );
		//	Debug.Log( nextPos );
		//	if( agent.transform.position.y < agent.nextPosition.y + jumpHeight )
		//	{
		//		Debug.Log( "점프해야함" );
		//	}
		//	else
	...
	void FixedUpdate( )
	{
		...
		//check if falling
		if( rb.velocity.y < fallingVelocity && !isFalling ) // && useNavMesh != true )
		{
			if( agent.isOnOffMeshLink )
			{
				isFalling = true;
